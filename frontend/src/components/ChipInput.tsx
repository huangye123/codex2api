  ...
  const handleChange = useCallback((e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (v.includes(',')) {
      ...
      for (let i = 0; i < parts.length - 1; i++) {
        if (parts[i].trim()) addChip(parts[i])
      }
    ...
    <div ref={containerRef} className={`relative ${className}`}>
      ...
      >
        ...
        <input
          ...
          onFocus={() => { if (hasOptions) setShowDropdown(true) }}
        ...
        {hasOptions && (
          <button
            type="button"
            onClick={(e) => { e.stopPropagation(); setShowDropdown(!showDropdown); inputRef.current?.focus() }}
        ...
        <div className="absolute z-50 mt-1 w-full max-h-48 overflow-auto rounded-md border border-border bg-popover shadow-md">
          {availableOptions.map((opt) => (
            <button
              ...
              onMouseDown={(e) => { e.preventDefault(); addChip(opt) }}
            >
              {opt}
            </button>
          ))}
